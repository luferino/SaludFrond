...
(Previously: the form rendered only the two required fields username and password.)
...
(Previously: the payload carried only username and password, errors were matched by substring against "409"/"400"/"duplicate"/"validation", and success redirected to the non-existent `/login`.)
